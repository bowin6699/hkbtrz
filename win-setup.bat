...
type "%USERPROFILE%\.ssh\id_rsa.pub" | ssh -o StrictHostKeyChecking=no %SERVER_USER%@%SERVER_HOST% "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"
if errorlevel 1 (
    echo FAILED. Check network or server address.
    pause
    exit /b
)
echo Done.
...
echo ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -N -R 2222:localhost:22 %SERVER_USER%@%SERVER_HOST% >> "%USERPROFILE%\.ssh\tunnel-loop.bat"
